...
def upgrade():
    ...
    if "aml_check" not in table_names:
    ...
    existing_indexes = {
        index["name"] for index in inspector.get_indexes("aml_check")
    }
    _create_index_if_missing(existing_indexes, "ix_aml_check_status", ["status"])
    _create_index_if_missing(
        existing_indexes,
        "ix_aml_check_deposit_decision",
        ["deposit_decision"],
    )
    _create_index_if_missing(
        existing_indexes, "ix_aml_check_next_retry_at", ["next_retry_at"]
    )
    _create_index_if_missing(
        existing_indexes, "ix_aml_check_created_at", ["created_at"]
    )


def _create_index_if_missing(existing_indexes, name, columns):
    if name not in existing_indexes:
        op.create_index(name, "aml_check", columns)
