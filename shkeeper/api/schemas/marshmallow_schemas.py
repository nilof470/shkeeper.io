...
class PayoutRequestSchema(Schema):
    ...
    fee = fields.String(
        required=False,
        description=(
            "Transaction fee (sat/vByte for BTC, sat/Byte for LTC/DOGE, "
            "integer for XMR, ignored for others). Required unless the crypto "
            "supports fee omission, for example USDT TRC-20 with resource preflight."
        ),
        example="10",
    )
...
class PayoutStatusResponseSchema(Schema):
    ...
    task_id = fields.String(
        example="b2a01bb0-8abe-403b-a3fa-8124c84bcf23",
        description="Async sidecar task ID, if the payout was queued",
    )
    success = fields.String(example="Yes", description="Legacy success marker")
    error = fields.String(
        example="Payout enqueue pending",
        description="Failure or reconciliation message, if any",
    )
    txids = fields.List(
        fields.String(),
        example=[
        ...
        description="All blockchain transaction IDs associated with this payout",
    )
    reconciliation_required = fields.Boolean(
        example=False,
        description=(
            "True when SHKeeper has a reserved payout row but cannot yet prove "
            "whether the sidecar enqueue or direct transaction completed."
        ),
    )
